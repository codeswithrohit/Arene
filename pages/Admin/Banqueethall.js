...
const HotelData = () => {
    ...
    const handleVerificationChange = async (id, Verfied) => {
        try {
            const db = firebase.firestore();
            const pgRef = db.collection('Banqueethalldetail').doc(id);
            await pgRef.update({ Verfied: Verfied });
            // Update the local state to reflect the change immediately
            setHoteldata(prevState => prevState.map(item => item.id === id ? { ...item, Verfied: Verfied === 'true' } : item));
            if (Verfied === 'true') {
    ...
    const filteredData = filter === 'approved' ? hoteldata.filter(item => item.Verfied) :
                        filter === 'unapproved' ? hoteldata.filter(item => !item.Verfied) :
                        hoteldata;
    ...
    return (
        <div className="min-h-screen bg-white dark:white">
            ...
            <h1 className="text-xl font-bold mb-4">Banqueet Hall Data</h1>
            <div className="mb-4">
            ...
            <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {filteredData.map((item, index) => (
                        <tr key={index}>
                            <td className="px-6 py-4 whitespace-nowrap">{item.BanqueethallName}</td>
