...
const handleVerificationChange = async (id, Verfied) => {
    try {
        ...
        await pgRef.update({ Verfied: Verfied }); // Change to 'Verfied'
        // Update the local state to reflect the change immediately
        setRentdata(prevState => prevState.map(item => item.id === id ? { ...item, Verfied: Verfied === 'true' } : item)); // Change to 'Verfied'
        if (Verfied === 'true') {
