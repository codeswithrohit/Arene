      ...
      useEffect(() => {
        const fetchData = async () => {
          try {
            ...
            const collectionRef = firebase.firestore().collection('buydetail').where('subcat', '==', subcategory).where('Verfied', '==', 'true');
...
>
            ...
            {item.imgSrc.map((src, idx) => (
              <div key={idx}>
                ...
                <div className="listing-content">
                  ...
                  {item.propertytypes && item.propertytypes.map((property, i) => (
  <div key={i}>
  ...
  {item.propertytypes.map((property, i) => (
                    <div key={i}>
                          <span className="price text-xs">{i + 1}. {property.type} - {property.price}</span>
                    </div>
                  ))}
